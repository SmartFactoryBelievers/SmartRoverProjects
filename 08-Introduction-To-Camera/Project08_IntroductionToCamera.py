...
for i in range(100): # loop through each number from 0 to 99
    camera.contrast = i # set camera contrast as the number of current loop
    camera.annotate_text = '%s' %i # display the number of contrast level
    sleep(0.1) # wait 0.1 second before moving to the next contrast level
camera.stop_preview() # stop the camera preview

camera.contrast = 75 	# Reset contrast
camera.brightness = 75	# Reset brightness

# For challenge 6, try iterating through IMAGE_EFFECTS, EXPOSURE_MODES, and AWB_MODES 
camera.start_preview() # start the camera
for effect in camera.IMAGE_EFFECTS: # For challenge 6, replace IMAGE_EFFECTS with EXPOSURE_MODES or AWB_MODES
    camera.annotate_text = '%s' %effect # display the name of the current effect
    camera.image_effect = effect # change the camera's image effect to the current value from the loop
    #camera.exposure_modes = mode # uncomment this line to iterate through EXPOSURE_MODES
    #camera.Aawb_modes = mode # uncomment this line to iterate through AWB_MODES
