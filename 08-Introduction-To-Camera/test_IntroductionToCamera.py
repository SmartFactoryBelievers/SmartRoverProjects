# test_IntroductionToCamera.py
# This is a simple test script to validate that Project08_IntroductionToCamera.py will execute as intended without throwing any error & the challenges are completed successfully.

# importing the subprocess module allows us to run other programs from this Python file
import subprocess 

# define a function called test_Project7
def test_Project8():
    # This function is execute any time we run the test
    try:
        # Run Project 8 and record the output
        result = subprocess.run(['python3', 'Project08_IntroductionToCamera.py'], check=True, capture_output=True, text=True) # check if there is any error in the output
...
# test challenge 1
'''
Write functions to test the challenges in Project 7
