...
while True: #Looping over and over again
    ...
    if GPIO.input(Button_Pin) == False: # For challenge 3, when the button is NOT pressed, blink LED
        sleep(LED_Off) # Keep LED off for defined duration
        GPIO.output(LED_Pin, GPIO.HIGH) # Turn LED on
        sleep(LED_On) # Keep LED on for defined duration
        GPIO.output(LED_Pin, GPIO.LOW) # Turn LED off
        
    # For challenge 3, if the button is pressed, the code will go to the else statement
    else:
        print('Button pressed') # For challenge 3, changed to 'Button pressed' 
        sleep(1)
